...
<template>
  <a :href="withBase(article.url)"
    ...
    <div
      class="grid grid-cols-[minmax(0,1fr)_auto] gap-x-4 gap-y-4 md:grid-cols-[8.5rem_minmax(0,1fr)_10.5rem] md:items-center md:gap-8">
      ...
      <div class="flex gap-6">
        <!-- <img v-if="article.frontmatter.banner" class="h-20 bg-contain bg-no-repeat bg-center"
          :src="withBase(`/banners/${article.frontmatter.banner}`)">
        </img> -->
      ...
      <div
        class="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-neutral-400 md:col-span-1 md:block md:text-right">
